...
async function getClickUpTasks(token) {
  try {
    ...
    return (tasks.tasks || []).slice(0, 50).map(t => ({
      ...
      due_date: t.due_date, list: t.list?.name, folder: t.folder?.name, url: t.url,
...
export default async function handler(req, res) {
  ...
  const prompt = `You are a personal task assistant for Rysiu at Klingit, a creative web agency. Extract every actionable item from the data below. Return ONLY a valid JSON array, no markdown, no explanation.
...
Each item must have:
{ "id": "unique-string", "title": "what needs doing max 120 chars", "detail": "who is waiting and context max 200 chars", "source": "slack or gmail or clickup", "category": "fire or email or client or clickup or internal", "urgency": "high or medium or low", "who": "person or null", "when": "date or null", "link": "url or null" }

Ignore: bot messages, newsletters, automated notifications, marketing emails, calendar accepts/declines, ClickUp/Loom/Slack marketing emails.
